...
TEST(SignedTriangleAreaTimes2, UnitRightTriangleCCW) {
    // CCW -> positive area -> 2× area of 0.5 triangle = 1.0
    EXPECT_DOUBLE_EQ(
        SignedTriangleAreaTimes2(kCcwTriangle[0], kCcwTriangle[1], kCcwTriangle[2]), 1.
    );
}

TEST(SignedTriangleAreaTimes2, UnitRightTriangleCW) {
    // CW -> negative area -> 2× area of 0.5 triangle = -1.0
    EXPECT_DOUBLE_EQ(SignedTriangleAreaTimes2(kCwTriangle[0], kCwTriangle[1], kCwTriangle[2]), -1.);
}

TEST(SignedTriangleAreaTimes2, LargeCoordinates) {
    // Scales correctly -> 100×100 right triangle -> 2× area = 10000.0
    EXPECT_DOUBLE_EQ(
        SignedTriangleAreaTimes2(Point2D{0., 0.}, Point2D{100., 0.}, Point2D{0., 100.}), 10000.
    );
}

TEST(SignedTriangleAreaTimes2, kCollinearShapeHorizontal) {
    EXPECT_DOUBLE_EQ(
        SignedTriangleAreaTimes2(Point2D{0., 0.}, Point2D{1., 0.}, Point2D{2., 0.}), 0.
    );
}

TEST(SignedTriangleAreaTimes2, kCollinearShapeVertical) {
    EXPECT_DOUBLE_EQ(
        SignedTriangleAreaTimes2(Point2D{0., 0.}, Point2D{0., 1.}, Point2D{0., 2.}), 0.
    );
}

TEST(SignedTriangleAreaTimes2, AllCoincident) {
    EXPECT_DOUBLE_EQ(
        SignedTriangleAreaTimes2(Point2D{1., 1.}, Point2D{1., 1.}, Point2D{1., 1.}), 0.
    );
}

TEST(SignedTriangleAreaTimes2, AntisymmetryProperty) {
    ...
    EXPECT_DOUBLE_EQ(SignedTriangleAreaTimes2(b, c, a), -SignedTriangleAreaTimes2(b, a, c));
    EXPECT_DOUBLE_EQ(SignedTriangleAreaTimes2(c, a, b), -SignedTriangleAreaTimes2(c, b, a));
}
